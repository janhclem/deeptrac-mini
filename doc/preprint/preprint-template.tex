...
*** General summary of lagrangian transport modeling ***

\subsection{Mass-transfer particle tracking}

Explanation of the Mass-transfer particle tracking algorithm ***.
...
Even without explicit constraint on the mass conservation, DeepMix improves the mass conservation steadily throughout the learning process leading to a final conservation error on the order of 0.1\% per particle. This corrobarates the hypothesis that DeepMix not only fits the training data arbirtary, but in a pysical meaningfull way. However, the conservation error is still dozents of orders larger than the numerical precision.  

*** TODO: Add fine tuning with mass conservation 
...
The code of \textsc{deeptrac-mini} is publicly available on GitHub (***) under GPL licence. Training's data and training itself can be reproduced with the help of the repositories scripts on a common desktop computer. 
